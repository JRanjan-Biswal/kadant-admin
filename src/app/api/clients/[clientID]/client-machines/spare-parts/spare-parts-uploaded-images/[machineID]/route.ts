...
export async function GET(
    ...
    try {
        ...
        const response = await fetch(
            `${process.env.NEXT_PUBLIC_API_URL}/spare-part-photos/${machineID}/${clientID}`,
            {
                method: 'GET',
                headers: {
...
export async function POST(
    ...
    try {
        ...
        const response = await fetch(
            `${process.env.NEXT_PUBLIC_API_URL}/spare-part-photos`,
            {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${currentUser.accessToken}`,
        ...
        return NextResponse.json({ message: 'Spare part image uploaded successfully' });
    } catch (error) {
        console.error('Error uploading spare part image:', error);
        return NextResponse.json(
            { error: 'Failed to upload spare part image' },
            { status: 500 }
        );
    }
}
