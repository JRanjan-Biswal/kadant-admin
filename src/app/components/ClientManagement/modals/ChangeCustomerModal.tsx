...
interface ChangeCustomerModalProps {
    ...
    onSave: (customer: string) => Promise<void>;
}

export default function ChangeCustomerModal({
    open,
    onOpenChange,
    currentCustomer,
    onSave,
        ...
        <Dialog open={open} onOpenChange={handleClose}>
            ...
            >
                <div className="bg-[#DFE6EC] border-b border-[#96A5BA] flex items-center justify-between h-[72px] px-6">
                    <h3 className="text-[#2D3E5C] text-xl font-bold leading-7">Change Customer</h3>
                    <button
                        type="button"
                        onClick={handleClose}
                ...
                <div className="p-6 flex flex-col gap-5">
                    <div className="flex flex-col gap-2">
                        <label className="text-[#6b7280] text-base leading-6">Current Customer</label>
                        <div className="bg-[#DFE6EC] border border-[#96A5BA] rounded-[10px] h-[46px] px-4 flex items-center">
                            <span className="text-[#1f2937] text-base">{currentCustomer || "Not set"}</span>
                        </div>
                    </div>

                    <div className="flex flex-col gap-2">
                        <label className="text-[#6b7280] text-base leading-6">
                            New Customer <span className="text-[#9ca3af] text-sm">(leave blank to clear)</span>
                        </label>
                        <input
                            type="text"
                            value={customer}
                            ...
                            placeholder="e.g. ANDAL, KCT, LATAM-MILLS"
